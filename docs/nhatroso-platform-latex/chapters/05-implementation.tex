...
\midrule
CI/CD Pipeline & Sử dụng GitHub Actions tự động chạy Test, Build Docker image và Deploy. \\
Máy chủ (Hosting) & Amazon EC2 chạy các Docker Container độc lập (\texttt{api}, \texttt{web}). \\
Cơ sở dữ liệu & Amazon RDS (PostgreSQL) đảm bảo lưu trữ an toàn, High Availability. \\
Object Storage & Amazon S3 lưu trữ hình ảnh đồng hồ và ảnh chứng từ OCR. \\
Web Server & Nginx đóng vai trò Reverse proxy, cấu hình bảo mật SSL Let's Encrypt. \\
OCR Engine & Google Cloud Vision API đảm nhận xử lý trích xuất chữ viết (OCR). \\
\bottomrule
\end{tabularx}
\end{table}
...
\textbf{Authentication Module:} Sử dụng JWT (JSON Web Token) với cơ chế Access và Refresh token riêng biệt. Mật khẩu được mã hóa an toàn bằng Argon2. Logic xác thực được triển khai qua các Loco Hook tích hợp sẵn.
...
\textbf{Meter \& Service Management:} Quản lý \texttt{room\_services} và \texttt{price\_rules}. Hỗ trợ tính giá linh hoạt.
