...
Hệ thống hỗ trợ đăng ký tài khoản bằng email hoặc số điện thoại với mật khẩu được bảo mật. Mỗi người dùng được cấp một mã định danh duy nhất kiểu \textbf{UUID}. Sau khi đăng nhập, hệ thống phát JWT access token ngắn hạn kết hợp refresh token. Hai vai trò được phân biệt rõ ràng: \textbf{Owner} (chủ nhà) có quyền quản lý toàn bộ tài nguyên của mình thông qua Web Dashboard, còn \textbf{Tenant} (người thuê) sử dụng Mobile App để gửi chỉ số và xem hóa đơn.
...
Triển khai & Triển khai tự động hoàn toàn qua \textbf{GitHub Actions}; ảo hóa với \textbf{Docker}; lưu trữ cơ sở dữ liệu bằng \textbf{PostgreSQL} và vận hành trên hệ sinh thái \textbf{AWS}. \\
\bottomrule
\end{tabularx}
\end{table}
