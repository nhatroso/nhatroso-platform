...
Hệ thống hỗ trợ triển khai linh hoạt thông qua Docker và hỗ trợ đa cơ sở dữ liệu.
...
\midrule
PostgreSQL / SQLite & SeaORM hỗ trợ dual-database (SQLite cho dev, Postgres cho prod). \\
Docker Compose & Đóng gói toàn bộ \texttt{api}, \texttt{web}, và \texttt{db}. \\
Object Storage & Lưu trữ hình ảnh đồng hồ (filesystem mount trong bản MVP). \\
Nginx & Reverse proxy, quản lý SSL và định tuyến domain. \\
Static Assets & Phục vụ trực tiếp từ server Rust thông qua Loco static mount. \\
\bottomrule
\end{tabularx}
\end{table}
