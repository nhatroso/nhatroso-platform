...
use crate::models::_entities::contracts::{ActiveModel as ContractActiveModel, Model as ContractModel};
use crate::dto::contracts::{CreateContractParams, ContractResponse};
use crate::repositories::contracts::ContractRepository;
use crate::models::_entities::users::{ActiveModel as UserActiveModel, Entity as Users};

pub struct ContractService;

impl ContractService {
    pub async fn list(db: &DatabaseConnection) -> Result<Vec<ContractResponse>> {
        let contracts = ContractRepository::list(db).await?;
        let mut results = Vec::new();
        for contract in contracts {
            let owner = ContractRepository::find_owner(db, contract.user_id).await?;
            let tenant = ContractRepository::find_tenant_by_contract(db, contract.id).await?;
            results.push(Self::map_to_response(contract, owner, tenant));
        }
        Ok(results)
    }

    pub async fn get_by_id(db: &DatabaseConnection, id: Uuid) -> Result<std::result::Result<ContractResponse, (StatusCode, &'static str)>> {
        let contract = ContractRepository::find_by_id(db, id).await?;
    ...
    pub async fn create(db: &DatabaseConnection, landlord_id: Uuid, params: CreateContractParams) -> Result<ContractModel> {
        ...
        let active_contract = ContractActiveModel {
        ...
        let res = ContractRepository::insert(db, active_contract).await?;

        // 4. Link tenant
        ContractRepository::link_tenant(db, contract_id, tenant_id).await?;

        Ok(res)
    }

    fn map_to_response(
        contract: crate::models::_entities::contracts::Model,
        owner: Option<crate::models::_entities::users::Model>,
        tenant: Option<crate::models::_entities::users::Model>,
    ) -> ContractResponse {
        ContractResponse {
            id: contract.id,
