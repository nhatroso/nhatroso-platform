...
impl MigrationTrait for Migration {
    async fn up(&self, m: &SchemaManager) -> Result<(), DbErr> {
        m.create_table(
            Table::create()
                ...
                .col(ColumnDef::new(Alias::new("auto_generate")).boolean().not_null().default(false))
                ...
                .foreign_key(
                    ForeignKey::create()
                        .name("fk-auto_invoice_configs-landlord")
                        .from(Alias::new("auto_invoice_configs"), Alias::new("landlord_id"))
    ...
    async fn down(&self, m: &SchemaManager) -> Result<(), DbErr> {
        m.drop_table(Table::drop().table(Alias::new("auto_invoice_configs")).to_owned()).await
    }
}
